...
export class Storage extends Driver
{
    ...
    prefixKey(key) {
        return 'prefix' in this._options ? this._options.prefix + key : key
    }
    ...
    async toRawValue(value, options = {}) {
        ...
        if (this._options.encrypt) {
            raw = await this._encryptor.encrypt(raw)
        }
        return raw
    }
    ...
    async fromRawValue(key, rawValue) {
        if (this._options.encrypt) {
            rawValue = await this._encryptor.decrypt(rawValue)
        }
        if (this._options.flatten) {
